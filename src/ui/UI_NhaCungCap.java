	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnSua)) {
			if (!txtMaNCC.getText().equals("")) {
				...
				JOptionPane.showMessageDialog(null, "Cần chọn nhà cung cấp trong bảng trước khi thay đổi.");
			}
	...
	private boolean ktra() {
		...
		if (hanhdong==1 || hanhdong==2) {
			...
			if (txtEmail.getText().equals("") || !txtEmail.getText().matches("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,6}$")) {
				txtThongBao.setText("Lỗi nhập liệu: Email nhập không đúng cú pháp. (Cần nhập theo đúng cú pháp email)");
				txtEmail.setBorder(new LineBorder(Color.RED, 1));
				kq=false;
			}
			if (txtDiaChi.getText().equals("") ) {
				txtThongBao.setText("Lỗi nhập liệu: Địa chỉ nhập không đúng. (không được rỗng)");
			...
			if (txtTenNCC.getText().equals("") || !(txtTenNCC.getText().length()>=1) || !(txtTenNCC.getText().length()<=64)) {
				txtThongBao.setText("Lỗi nhập liệu: Tên nhà cung cấp nhập không đúng. (từ 1 - 64 ký tự)");
